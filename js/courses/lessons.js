...
export const LESSON_STEPS = [
  ...
  {
    text: '<code>R</code> ゲートは「位相」を回すゲート(タップするたびに45°ずつ回って角度が変わる)。H のうしろ(右隣)に R を置いても、音=確率はまったく変わらない。でもメーター右下の色ダイヤル(位相)だけが回る。この見えない性質が量子計算のカギ。',
    allowed: ['X', 'H', 'R'],
    check: (c) => c.counts.R >= 1 && c.counts.H >= 1 && [...c.phases].some((ph) => Math.abs(ph) > 0.1),
  },
  {
    text: '位相は「干渉」で威力を発揮する。リセットして q0 の行に <code>H → R(π) → H</code> と並べてみよう(R はタップして π に合わせる)。2回目の H で波が打ち消し合い、X を使っていないのに曲が切り替わる!<span class="goal">目標: X を使わずに曲 100「チルアウト」を100%に</span>',
    allowed: ['X', 'H', 'R'],
